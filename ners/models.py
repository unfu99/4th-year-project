...
def pretrain_template_uv_with_z(
    template_uv,
    verts=None,
    faces=None,
    extents=None,
    ...
    """
    Pretrains the template UV shape model, with a dummy variable z. Must be initialized either with vertices and
...
class TemplateUVZ(BaseNetwork):
    def __init__(
        self, num_layers=3, input_size=3, output_size=3, hidden_size=256, L=10, D=64 #z parameter here D=64
    ):
        super().__init__(n_harmonic_functions=L)
        input_size = L * 2 * input_size + D
        self.z = nn.parameter.Parameter(data=torch.randn(1, D), requires_grad=True)
        layers = []
        for i in range(num_layers - 1):
            if i == 0:
                layers.append(nn.Linear(input_size, hidden_size))
    ...
    def forward(self, x, z=None, normalize=True):
        if z is None:
            z = self.z
        temp_device = x.device
        x = x.to(self.get_device(temp_device))
...
class Symmetrize(BaseNetwork):
    ...
    def forward(self, uvs, *args, **kwargs):
        uvs_ref = uvs * 1

        uvs_ref[..., [self.sym_axis]] *= -1
        pred = self.uv3d(uvs, *args, **kwargs)
        pred_ref = self.uv3d(uvs_ref, *args, **kwargs)
...
def load_car_model_with_z(path="models/templates/car_with_z.pth"):
    template = TemplateUVZ(L=10, num_layers=3, hidden_size=256, D=64)
    template.load_state_dict(torch.load(path))
    return template
