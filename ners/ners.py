...
class Ners(object):
    def __init__(
        ...
        for data in data_list:
            scene_ids.append(data["id"])
            images = data["images"]
            N_list.append(len(images))
            images_list.append(images)
            masks_list.append(data["masks"])
            masks_dt_list.append(data["masks_dt"])
            initial_poses_list.append(data["initial_poses"])
            image_center_list.append(data["image_centers"])
            crop_scale_list.append(data["crop_scales"])   
        ...
        all_images_tensor_list = []
        for all_imgs in all_images_list:
            tensor = torch.from_numpy(all_imgs.transpose(0, 3, 1, 2))
            tensor = tensor.float()#.to(device).contiguous()
            all_images_tensor_list.append(tensor)
        self.all_images_tensor_list = all_images_tensor_list
        ...
        for masks in masks_list:
            target_masks = torch.from_numpy(np.stack(masks)).float()#.to(device)
            target_masks_list.append(target_masks)
        target_masks_dt_list = []
        for masks_dt in masks_dt_list:
            target_masks_dt = torch.from_numpy(np.stack(masks_dt)).float()#.to(device)
            target_masks_dt_list.append(target_masks_dt)
        self.target_masks_list=target_masks_list
        self.target_masks_dt_list=target_masks_dt_list
        ...
        images_masked_list = []
        for i in range(0, self.num_scenes):
            images_masked = all_images_tensor_list[i].clone()
            images_masked[self.target_masks_list[i].unsqueeze(1).repeat(1, 3, 1, 1) == 0] = 1
            images_masked_list.append(images_masked)

        self.images_masked_list = images_masked_list
        self.jitter_uv = jitter_uv
        self.mean_texture_list = [1] * self.num_scenes
        self.scene_ids = scene_ids
        z=f_template.z.repeat(self.num_scenes, 1).clone().detach().requires_grad_(True)
        # z_tensor=torch.tensor(data=z, requires_grad=True)
        self.z_parameters = nn.parameter.Parameter(data=z.cuda().requires_grad_(), requires_grad=True).to(device)
        # self.z_parameters = nn.parameter.Parameter(data=f_template.z.repeat(self.num_scenes, 1)).to(device)
        self.z_parameters.requires_grad = True
        ...
        for i in range(0, self.num_scenes):
            ...
            fov=self.fov_list[i],
            R=np.stack(initial_poses_list[i]),
            T=[[0, 0, 2]] * self.N_list[i],
            image_center=image_center_list[i],
            crop_scale=crop_scale_list[i],
            device=device,
            )
        ...
        for i in range(0, self.num_scenes):
            ...
            pred_vs, sv = self.get_pred_verts(scene_id, None, self.sphere_vs, predict_deformation=False)
            texture_rainbow = TexturesVertex((sv.unsqueeze(0) + 1) / 2)
            mesh = Meshes(
            [pred_vs], [self.sphere_fs], textures=texture_rainbow
            )
        ...
        edge_counts_list = []
        for i in range(0, self.num_scenes):
            masks_pooled = -pool(-self.target_masks_list[i])  # Min pool
            edges = self.target_masks_list[i] - masks_pooled
            edge_pixels = [torch.stack(torch.where(e > 0.5), dim=1) for e in edges]
            edge_counts = [e.shape[0] for e in edge_pixels]
            edge_pixels_list.append(edge_pixels)
            edge_counts_list.append(edge_counts)
        # [(E, 2)]
        self.edge_pixels_list = edge_pixels_list
        self.edge_counts_list = edge_counts_list
        ...
        # for i in range(0, self.num_scenes):
        #     loss_perceptual_list.append(DataParallel(lp, device_ids=gpu_ids).to(device))
        # self.loss_perceptual_list = loss_perceptual_list
    ...
    def get_z_parameter(self, scene_id):
        z=self.z_parameters[self.scene_ids.index(scene_id), ...]
        return z
            
    def get_pred_verts(self, scene_id, z=None, sphere_vs=None, predict_deformation=True, jitter_uv=None):
        """
        ...
        if self.use_template_as_shape:
            if z is None:
                z = self.get_z_parameter(scene_id=scene_id)
            z = z.expand(sv.size(0), -1)
            pred_vs = self.f_template(sv, z=z)
    ...
    def compute_chamfer_loss(self, scene_id, meshes, cameras, num_samples=300):
        """
        ...
        for counts, pixels in zip(self.edge_counts_list[scene_id], self.edge_pixels_list[scene_id]):
        ...
        # (N, num_samples, 1, 2)
        edge_pixels_sampled = torch.stack(edge_pixels_sampled).unsqueeze(2).to(self.device)
        # Projected Points:
        proj = cameras.transform_points_screen(  # (N, V, 2)
            meshes.verts_padded(),
            image_size=[(256, 256)] * self.N_list[scene_id],
        )
    ...
    def visualize_input_views(self, output_dir=None, filename=None, title="", meshes=None, cameras=None):
        """
        Generates visualizations for each of the training views.

        Args:
            output_dir (str): If not None, saves the image to this file.
        ...
        for scene_id in range(0, self.num_scenes):
            # if meshes is None:
            #     meshes = self.meshes_list[scene_id].extend(self.N_list[scene_id])
            # if cameras is None:
            #     cameras = self.camera_list[scene_id]
            meshes = self.meshes_list[scene_id].extend(self.N_list[scene_id])
            cameras = self.camera_list[scene_id]
            with torch.no_grad():
                rend = self.renderer_textured(meshes, cameras=cameras)
            ...
            axs = axs.flatten()
            for i in range(n):
                image = self.all_images_list[scene_id][i]
                ...
                mask_pred = self.masks_list[scene_id][i] > 0.5
            ...
            if output_dir:
                id=self.scene_ids[scene_id]
                fname=osp.join(output_dir, f"{id}{filename}")
                plt.savefig(fname=fname, format="jpg", bbox_inches="tight")
                plt.close()
            else:
                plt.show()
    ...
    def optimize_camera(self, num_iterations=500, finetune_rotations=True, pbar=True):
        ...
        optim_list = []
        for i in range(0, self.num_scenes):
            _T = camera_list[i].T.detach()
            _R = geom_util.matrix_to_rot6d(camera_list[i].R.detach())
            _T.requires_grad = True
            _R.requires_grad = False
            _meshes = self.meshes_list[i].extend(self.N_list[i]).detach()
            _optim = torch.optim.Adam([_T], lr=0.01)
            T_list.append(_T)
            R_list.append(_R)
            meshes_list.append(_meshes)
            optim_list.append(_optim)

        loop = tqdm(range(num_iterations)) if pbar else range(num_iterations)
        for i in loop:
            scene_id = random.randint(0, self.num_scenes-1)
            # cameras=camera_list[scene_id]
            # T=T_list[scene_id]
            # R=R_list[scene_id]
            # meshes=meshes_list[scene_id]
            # optim=optim_list[scene_id]
            target_masks = self.target_masks_list[scene_id].to(self.device)
            target_masks_dt = self.target_masks_dt_list[scene_id].to(self.device)
            optim_list[scene_id].zero_grad()
            camera_list[scene_id].T = T_list[scene_id]
            camera_list[scene_id].R = geom_util.rot6d_to_matrix(R_list[scene_id])
            rend = self.renderer_silhouette(meshes_list[scene_id], cameras=camera_list[scene_id])[..., 3]
            losses = {"silhouette": torch.mean((rend - target_masks) ** 2)}
            losses["chamfer"] = self.compute_chamfer_loss(
                scene_id=scene_id, meshes=meshes_list[scene_id], cameras=camera_list[scene_id]
            )
            losses["distance_transform"] = torch.mean(rend * target_masks_dt)
            losses["offscreen"] = self.compute_offscreen_loss(
                camera_list[scene_id], meshes_list[scene_id].verts_padded().detach()
            )
            loss = sum(losses[k] * self.loss_weights_list[scene_id][k] for k in losses.keys())
            if pbar:
                loop.set_description(f"Camera: {loss.item():.4f}")
            loss.backward()
            optim_list[scene_id].step()
            if i == num_iterations // 2 and finetune_rotations:
                R_list[scene_id].requires_grad = True
                optim_list[scene_id] = torch.optim.Adam([T_list[scene_id], R_list[scene_id]], lr=1e-3)

        for i in range(0, self.num_scenes):
            R_list[i].requires_grad = False
            T_list[i].requires_grad = False
            self.camera_list[i] = camera_list[i].detach()
    ...
    def optimize_shape(self, num_iterations=500, pbar=True, lr=1e-4):
        ...
        optim_list = []
        parameters_list = []
        pred_vs_list = [1] * self.num_scenes
        sv_list = [1] * self.num_scenes
        loss_dict_list = []
        z_parameters_list = []
        for i in range(0, self.num_scenes):
            _T = camera_list[i].T.detach()
            ...
            _T.requires_grad = True
            _R.requires_grad = False
            _fov = self.fov_list[i].clone().detach()
            _z = self.get_z_parameter(self.scene_ids[i])
            _z = torch.tensor(_z, requires_grad = True)
            _z.requires_grad = True
            params = [_R, _T, _fov]
            for param in params:
                param.requires_grad = True
            _parameters = [{"params": params, "lr": lr * 10}]
            _parameters.append({"params": self.f_shape.parameters(), "lr": lr})
            _parameters.append({"params": _z, "lr": lr * 10})
            _optim = torch.optim.Adam(_parameters)
            _loss_dict = {}
            T_list.append(_T)
            R_list.append(_R)
            fov_list.append(_fov)
            z_parameters_list.append(_z)
            parameters_list.append(_parameters)
            optim_list.append(_optim)
            loss_dict_list.append(_loss_dict)
        ...
        for _ in loop:
            ...
            target_masks_dt = self.target_masks_dt_list[scene_id].to(self.device)
            optim_list[scene_id].zero_grad()
            pred_vs_list[scene_id], sv_list[scene_id] = self.get_pred_verts(predict_deformation=True, z=z_parameters_list[scene_id], scene_id=self.scene_ids[scene_id])
            meshes = Meshes([pred_vs_list[scene_id]], [self.sphere_fs]).extend(self.N_list[scene_id])
            camera_list[scene_id] = PerspectiveCameras(
                fov=fov_list[scene_id],
                R=geom_util.rot6d_to_matrix(R_list[scene_id]),
                T=T_list[scene_id],
                image_center=self.camera_list[scene_id].image_center,
                crop_scale=self.camera_list[scene_id].crop_scale,
                device=self.device,
            )
            rend = self.renderer_silhouette(meshes, cameras=camera_list[scene_id])[..., 3]
            loss_dict_list[scene_id]["silhouette"] = torch.mean((rend - target_masks) ** 2)
            loss_dict_list[scene_id]["distance_transform"] = torch.mean(rend * target_masks_dt)
            loss_dict_list[scene_id]["offscreen"] = self.compute_offscreen_loss(
                camera_list[scene_id], pred_vs_list[scene_id].unsqueeze(0).detach()
            )
            loss_dict_list[scene_id]["laplacian"] = mesh_laplacian_smoothing(meshes)
            loss_dict_list[scene_id]["normal"] = mesh_normal_consistency(meshes)
            loss_dict_list[scene_id]["chamfer"] = self.compute_chamfer_loss(scene_id, meshes, camera_list[scene_id])
            loss = sum(loss_dict_list[scene_id][k] * self.loss_weights_list[scene_id][k] for k in loss_dict_list[scene_id].keys())
            if pbar:
                loop.set_description(f"Shape: {loss.item():.4f}")
            wandb.log({f"{scene_id}_total_loss": loss, 
            f"{scene_id}_silhouette" : loss_dict_list[scene_id]["silhouette"],
            f"{scene_id}_distance_transform" : loss_dict_list[scene_id]["distance_transform"],
            f"{scene_id}_offscreen" : loss_dict_list[scene_id]["offscreen"],
            f"{scene_id}_laplacian" : loss_dict_list[scene_id]["laplacian"],
            f"{scene_id}_normal" : loss_dict_list[scene_id]["normal"],
            f"{scene_id}_chamfer" : loss_dict_list[scene_id]["chamfer"],
            })
            loss.backward()
            optim_list[scene_id].step()
            
        z_tensor=torch.zeros(self.num_scenes,self.z_dim)
        for i in range(0, self.num_scenes):
            self.fov_list[i]=fov_list[i].clone().detach()
            self.camera_list[i] = camera_list[i].detach()
            self.meshes_list[i] = Meshes(
                verts=[pred_vs_list[i]],
                faces=[self.sphere_fs],
                textures=TexturesVertex((sv_list[i].unsqueeze(0) + 1) / 2),
            ).detach()
            z_tensor[i,:] = z_parameters_list[i]

        self.z_parameters=nn.parameter.Parameter(data=z_tensor.cuda().requires_grad_(), requires_grad=True)
    ...
    def optimize_texture(self, num_iterations=3000, lr=1e-4, pbar=True):
        ...
        pred_vs_list = [1] * self.num_scenes
        sv_list = [1] * self.num_scenes
        meshes_list = [1] * self.num_scenes
        pred_textures_list = [1] * self.num_scenes
        loss_dict_list = []
        for i in range(0, self.num_scenes):
            _T = camera_list[i].T.detach()
            _R = geom_util.matrix_to_rot6d(camera_list[i].R.detach())
        ...
        for _ in loop:
            ...
            pred_vs_list[scene_id], sv_list[scene_id] = self.get_pred_verts(predict_deformation=True, scene_id=self.scene_ids[scene_id])
            # pred_vs=pred_vs_list[scene_id]
            # sv=sv_list[scene_id]
            meshes_list[scene_id] = Meshes([pred_vs_list[scene_id]], [self.sphere_fs]).extend(self.N_list[scene_id])
            pred_textures_list[scene_id] = TexturesImplicit(
                texture_predictor=self.f_tex,
                faces=self.sphere_fs,
                verts_sphere_coords=sv_list[scene_id],
                verts_deformed_coords=pred_vs_list[scene_id],
                predict_radiance=False,
            )
            meshes_list[scene_id].textures = pred_textures_list[scene_id]
            rend_sil = self.renderer_silhouette(meshes_list[scene_id], cameras=camera_list[scene_id])[..., 3]
            rend_text = self.renderer_textured(meshes_list[scene_id], cameras=camera_list[scene_id])[..., :3]
            rend_text = rend_text.clamp(0, 1)
            loss_dict_list[scene_id]["silhouette"] = torch.mean((rend_sil - target_masks) ** 2)
            loss_dict_list[scene_id]["distance_transform"] = torch.mean(
                rend_sil * target_masks_dt
            )
            loss_dict_list[scene_id]["offscreen"] = self.compute_offscreen_loss(
                camera_list[scene_id], pred_vs_list[scene_id].unsqueeze(0).detach()
            )
            loss_dict_list[scene_id]["laplacian"] = mesh_laplacian_smoothing(meshes_list[scene_id])
            loss_dict_list[scene_id]["normal"] = mesh_normal_consistency(meshes_list[scene_id])
            loss_dict_list[scene_id]["texture"] = self.loss_perceptual(
                self.images_masked_list[scene_id], rend_text.permute(0, 3, 1, 2)
            ).mean()
            loss_dict_list[scene_id]["chamfer"] = self.compute_chamfer_loss(scene_id, meshes_list[scene_id], camera_list[scene_id])
            loss = sum(loss_dict_list[scene_id][k] * self.loss_weights_list[scene_id][k] for k in loss_dict_list[scene_id])
            if pbar:
        ...
        for i in range(0, self.num_scenes):
            self.fov_list[i]=fov_list[i].clone().detach()
            self.camera_list[i] = camera_list[i].detach()
            self.meshes_list[i] = meshes_list[i][0]
            self.meshes_list[i].textures = pred_textures_list[i]
    ...
    def optimize_radiance(self, num_iterations=500, pbar=True, lr=1e-4):
        ...
        meshes_list = [1] * self.num_scenes
        pred_textures_list = [1] * self.num_scenes
        mean_texture_list = [1] * self.num_scenes
        loss_dict_list = []
        for i in range(0, self.num_scenes):
            _T = camera_list[i].T.detach()
            _R = geom_util.matrix_to_rot6d(camera_list[i].R.detach())
            _fov = self.fov_list[i].clone().detach()
            params = [_R, _T, _fov, self.specularity, self.shininess]
            for param in params:
                param.requires_grad = True
            _parameters = [{"params": params, "lr": lr * 10}]
            _parameters.append({"params": self.f_shape.parameters(), "lr": lr})
            _parameters.append({"params": self.f_tex.parameters(), "lr": lr})
            _parameters.append({"params": self.f_env.parameters(), "lr": lr})
            _optim = torch.optim.Adam(_parameters)
            _loss_dict = {}
            T_list.append(_T)
            R_list.append(_R)
        ...
        for _ in loop:
            ...
            pred_vs_list[scene_id], sv_list[scene_id] = self.get_pred_verts(predict_deformation=True)
            meshes_list[scene_id] = Meshes([pred_vs_list[scene_id]], [self.sphere_fs]).extend(self.N_list[scene_id])
            pred_textures_list[scene_id] = TexturesImplicit(
                texture_predictor=self.f_tex,
                faces=self.sphere_fs,
                verts_sphere_coords=sv_list[scene_id],
                verts_deformed_coords=pred_vs_list[scene_id],
                verts_normals=meshes_list[scene_id].verts_normals_padded()[0],
            ...
            pred_textures_list[scene_id].custom_device = self.device_illumination
            mean_texture_list[scene_id] = self.f_tex(sv_list[scene_id]).mean(dim=0).clone().detach()
            meshes_list[scene_id].textures = pred_textures_list[scene_id]
            rend_sil = self.renderer_silhouette(meshes_list[scene_id], cameras=camera_list[scene_id])[..., 3]
            rend_text = self.renderer_textured(meshes_list[scene_id], cameras=camera_list[scene_id])[..., :3]
            rend_text = rend_text.clamp(0, 1)
            pred_textures_list[scene_id].set_texture_color(mean_texture_list[scene_id])
            rend_text_mean = self.renderer_textured(meshes_list[scene_id], cameras=camera_list[scene_id])
            rend_text_mean = rend_text_mean[..., :3].clamp(0, 1)
            pred_textures_list[scene_id].texture_color = None
            ...
            loss_dict_list[scene_id]["texture_mean"] = self.loss_perceptual(
                self.images_masked_list[scene_id], rend_text_mean.permute(0, 3, 1, 2)
            ).mean()
            loss_dict_list[scene_id]["chamfer"] = self.compute_chamfer_loss(scene_id, meshes_list[scene_id], camera_list[scene_id])
        ...
        for i in range(0, self.num_scenes):
            ...
            self.meshes_list[i] = meshes_list[i][0]
            self.meshes_list[i].textures = pred_textures_list[i]
            self.mean_texture_list[i]=mean_texture_list[i].clone().detach()
    ...
    def load_parameters(self, filename, device=None):
        ...
        self.f_shape.load_state_dict(state_dicts["f_shape"])
        self.f_template.load_state_dict(state_dicts["f_template"])
        self.z_parameters = state_dicts["z_parameters"]
    ...
    def save_obj(
        ...
        with torch.no_grad():
            pred_verts, sv = self.get_pred_verts(scene_id, None, sv, jitter_uv=False)
